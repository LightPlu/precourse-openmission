...
- DB에 대하여 테스트 코드를 작성하려 하는데 DB와 관련된건 어떻게 테스트를 해애햐지? 실제 DB와 연결해서 하나하나 확인하는 것 밖에 없나?
  - 찾아보던 도중 @TestContainer라는 어노테이션에 대해 찾아보았다. 해당 내용을 적용해보기 위하여 예전에 다운만 받아놓았던 Docker를 켜서 한번 사용해봤다.
  - Docker에 대해서는 자세히 모르기 때문에 추후 한번 공부해볼 예정.


- 해당 과제에서 애그리거트 루트가 Round만 될 줄 알았는데 발표를 준비하고 개념을 복습하는 과정에서 LottoTicket또한 애그리거트 루트가 되는 것을 확인하였다. 또한 코드를 많이 수정해야 하는것을 발견했는데 어떻게 할 것인가?
  - 현재 애그리거트 루트와 관련하여 안되어있는 것들이 정말 많은것을 다시 발견. 우선 레포지토리를 애그리거트 루트 별로 분리한 뒤 각 레포지토리가 애그리거트 루트를 반환하도록 수정해야함.
  - 우선 발표자료를 만들다 발견한 부분이니 발표자료를 잠시 스탑하고 코드 리팩토링부터 우선시하여 수정하도록 함.
  - 발표는 내가 정확히 알고있는 상태에서 발표하는것이 맞다고 생각하므로 코드 리팩토링이 우선시라고 판단했음.